...
def add_event_component(item):
    ...
    ev.add("dtstamp", item["stamp"] or NOW)
    ...
    if isinstance(st, datetime):
        ...
        if not isinstance(end_date, date) or end_date <= st:
            end_date = st
...
def build():
    ...
    cal.add("refresh-interval", "PT1H", parameters={"VALUE": "DURATION"})
    ...
    removed = len(existing) + len(scraped) - len(merged)
