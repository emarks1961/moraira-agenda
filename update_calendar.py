...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; MorairaCalendarBot/4.0; +https://moraira-agenda.netlify.app/)"
}
...
def event_dict(title, start, end=None, location="", description="", url="",
               source="", priority="B"):
    title = clean(title)
...
def extract_jsonld(soup, source):
    ...
    for tag in soup.find_all("script", type=lambda v: v and "ld+json" in v):
        ...
        for item in expanded:
            ...
            # IMPORTANT: prefer the event's own URL.
            # Do not silently use the source overview page as the event URL.
            raw_url = item.get("url")
            url = clean_url(raw_url, source["url"]) if raw_url else ""

            e = event_dict(
                item.get("name"), start, end, loc, desc, url,
...
def extract_links_with_jsonld(soup, source, limit=60):
    ...
    domain_match = re.match(r"^https?://([^/]+)", source["url"])
    domain = domain_match.group(1).lower() if domain_match else ""

    for a in soup.find_all("a", href=True):
        href = clean_url(a["href"], source["url"])
        text = clean(a.get_text(" "))
        if not href or href in seen or domain not in href.lower() or len(text) < 4:
            continue

        # Follow likely individual event pages, but not generic source pages.
        path = href.lower()
        if not any(k in path for k in ["/evento", "/event", "/agenda", "/agendas/"]):
            continue

        # Avoid re-fetching the exact source page.
        ...
        try:
            ...
            child_source["url"] = href
            child_events = extract_jsonld(get_soup(href), child_source)

            # For a detail page, if JSON-LD lacks an event URL, use the detail
            # page itself. This is the correct individual activity URL.
            for e in child_events:
                if not e["url"]:
                    e["url"] = href
            found.extend(child_events)
        except Exception:
            pass
...
def extract_xabia_text(soup, source):
    lines = [
        clean(x) for x in soup.get_text("
", strip=True).splitlines()
    ...
    for i, line in enumerate(lines):
        if re.match(
            ...
            # Try to associate the title with a nearby link.
            url = ""
            for a in soup.find_all("a", href=True):
                if clean(a.get_text(" ")) == current_title:
                    url = clean_url(a["href"], source["url"])
                    break

            found.append(
                event_dict(
                    current_title, st, en, loc, "",
                    url or source["url"], source["name"], source["priority"]
                )
            )
            ...
            and not line.lower().startswith(
                ("hora ", "zona:", "precio", "calendario", "eventos en")
            )
        ):
            if len(line) <= 140:
                current_title = line
...
def extract_benitatxell_cards(soup, source):
    ...
    for h in soup.find_all(["h2", "h3", "h4"]):
        ...
        if not title or title.lower() in (
            "próximos eventos", "upcoming events", "agenda"
        ):
            continue
        ...
        for d, mon in pairs[:2]:
            ...
            if month:
                try:
                    dates.append(
                        datetime(
                            year, month, int(d), 10, 0, tzinfo=TZ
                        )
                    )
                except ValueError:
                    pass
        ...
        url = clean_url(a["href"], source["url"]) if a else source["url"]

        found.append(
            event_dict(
                title, st, en, source["place"], "", url,
                source["name"], source["priority"]
            )
        )
...
def collect():
    ...
    for source in SOURCES:
        try:
            ...
            # First use structured events on the overview page.
            all_events.extend(extract_jsonld(soup, source))

            # Then inspect likely individual event pages.
...
def selection_score(item):
    ...
    if item["category"] != "Overig evenement":
        score += 3

    if any(k in s for k in MAJOR_WORDS):
        score += 4
    ...
    if item["category"] == "Kunst & cultuur":
        score += 1

    if any(k in s for k in LOW_VALUE_WORDS):
        score -= 5
...
def keep_by_priority(item):
    p = item.get("priority", "B")
    s = text_blob(item)
    low = any(k in s for k in LOW_VALUE_WORDS)

    if p == "A":
        return not (low and item["category"] == "Overig evenement")

    if p == "B":
        return item["category"] != "Overig evenement" and not low
...
def dedupe(items):
    ...
    for x in items:
        if not is_relevant_future(x) or not keep_by_priority(x):
            continue

        key = (
            re.sub(r"[^a-z0-9]+", "", x["title"].lower())[:90],
            as_date_key(x["start"]).isoformat()
        )

        priorities = {"A": 3, "B": 2, "C": 1}
        ps = priorities.get(x.get("priority"), 1)

        # Prefer the item with the most specific URL when priorities tie.
        specificity = 1 if x.get("url") else 0
        candidate_score = (ps, specificity)

        if key not in chosen or candidate_score > chosen[key][0]:
            chosen[key] = (candidate_score, x)
...
def add_event_component(item):
    ...
    place = (
        item["location"].split(",")[-1].strip()
        if item["location"] else ""
    )

    title = f"{item['emoji']} {item['title']}"
    if place and place.lower() not in item["title"].lower():
        title += f" · {place}"
    ...
    # Only add URL when it is a verified HTTP(S) URL.
    if item["url"]:
        ev.add("url", item["url"])
    ...
    if isinstance(st, datetime):
        ev.add(
            "dtend",
            en if isinstance(en, datetime) and en > st
            else st + timedelta(hours=2)
        )
    else:
        ev.add(
            "dtend",
            (en if isinstance(en, date) else st) + timedelta(days=1)
        )
...
def existing_is_bot_event(comp):
    ...
    return "Bron:" in desc and (
        "Meer informatie:" in desc or "Categorie:" in desc
    )
...
def existing_location_priority(comp):
    ...
    if "moraira" in loc or "teulada" in loc:
        return "A"
    if "benissa" in loc or "benitatxell" in loc:
        return "B"
    if (
        "xàbia" in loc or "xabia" in loc
        or "calp" in loc or "calpe" in loc
    ):
        return "C"
...
def build():
    ...
    # Preserve manually maintained A-location events.
    # Remove old automatically generated B/C entries so stale/wrong URLs
    # are not retained.
    ...
    for k, v in [
        ("prodid", "-//Moraira Calendar//NL"),
        ("version", "2.0"),
        ("calscale", "GREGORIAN"),
        ("method", "PUBLISH"),
        ("x-wr-calname", "Moraira & omgeving – Evenementen"),
    ...
    with_url = sum(1 for x in found if x.get("url"))

    print(
        f"Klaar: {len(found)} geselecteerd "
        f"({a_count} Moraira/Teulada, "
        f"{len(found)-a_count} omgeving); "
        f"{with_url} met individuele URL; "
        f"{len(keep)} totaal."
    )
